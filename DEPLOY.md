...
## Newark (Silliman Center) — known limitation

Facebook detects GitHub Actions runner IPs as bots and serves a generic
marketing image instead of the real Newark Rec page. Major proxy services
(ScraperAPI, ScrapingBee, ZenRows) explicitly block Facebook in their
Terms of Service, so there's no drop-in proxy fix.

**What happens on every cron / Refresh run:** the Newark scraper detects
the placeholder, returns 0 sessions, and the defensive-write in
`scripts/run_all_scrapers.py` preserves the previously-committed Newark
data. The other three sources (AFG, RMCC, PCC) refresh normally.
**The workflow always succeeds** — a single scraper failure doesn't block
the merge + commit of the other three.

**To refresh Newark data:** run `python scripts/serve.py` on any machine
with a residential IP (your laptop, a Raspberry Pi, etc.), click Refresh
in the browser, then `git push`. Or set up a [self-hosted GitHub Actions
runner](https://docs.github.com/en/actions/hosting-your-own-runners) on
a home machine for fully automated Newark refreshes.
